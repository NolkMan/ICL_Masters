...
This project focuses on protecting users from script based attacks.
OWASP fundation classified Cross Site Scripting as the sevents most critical security risk in their OWASP Top 10 report for year 2017 \cite{owaspTop102017}.
...
Through XSS attackers gain control of the scripting engine and through the programmable interface they may try to achieve different goals.

\subsubsection{Steal session cookies}
If session cookies are not HttpOnly, which prevents them from being accessible by scripts, the attackers may retreive them in many different ways.
By doing that they will be able to authenticate themselves as the victim and perform any actions they could do, untill the cookie expires.
This gives the attackers total control over the account, where they can extorn money or sensitive information.
Banks additionally protect their clients by requiring additional 2 factor authentication before transfers.
...
They may perform requests in the name of the user to the host even when HttoOnly flag is set.
The browser will attach the session cookie to the request automatically.
In this way attackers' script may retreive or even change the users data.

\subsubsection{Deface or modify the application}
The attacker may also change the website to trick the user into inputing their credentials if the attack is only available on a specific part of the website.
As in other goals, the attacker eventually obtaines the full access to the account.
...
To achieve this goal they may remove all content from the site and insert offensive imagery.
